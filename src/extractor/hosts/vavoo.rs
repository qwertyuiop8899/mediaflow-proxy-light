...
//!
//! Two resolution paths are attempted:
...
/// Static `vec` payload required by `ping2` — the server validates this
/// exact value, so it must not be altered.
...
impl Extractor for VavooExtractor {
    ...
    async fn extract(
        ...
        headers.insert("X-MFP-Disable-SSL".to_string(), "1".to_string());
